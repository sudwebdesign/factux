   ...
   <center>
    <form name="formu" method="post" action="fact_bon_orph_suite.php" >
     <table width="460" border="0" class="page" align="center">
      <caption><?php echo sprintf('%s %s %s', $lang_cré_fac_orph, $lang_numero, $num); ?></caption>
      <tr>
       <td class="texte0"><?php echo "date" ?></td>
       <td class="texte0">
        <input type="text" name="date_fact" value="<?php echo sprintf('%s/%s/%s', $jour, $mois, $annee) ?>" readonly="readonly"/>
...
$date = $row["date"];
?>
          <option value='<?php echo $num_bon; ?>'><?php echo sprintf('%s %s du %s %s %s %s', $lang_num_bon_ab, $num_bon, $date, $lang_pour_mont, $tot, $devise); ?></option>
<?php } ?>
         </select>
        </td>
      </tr>
