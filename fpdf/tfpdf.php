...
public $CurOrientation;
     ...
     // current orientation
/**
 * @var array{a3: array{841.89, 1190.55}, a4: array{595.28, 841.89}, a5: array{420.94, 595.28}, letter: array{612, 792}, legal: array{612, 1008}}
 */
public $StdPageSizes;
...
/**
 * @var array{'courier', 'helvetica', 'times', 'symbol', 'zapfdingbats'}
 */
public $CoreFonts;
...
public $DrawColor = '0 G';

          // commands for drawing color
public $FillColor = '0 g';

          // commands for filling color
public $TextColor = '0 g';
...
{
	__construct($orientation, $unit, $size);
}
...
public function SetDisplayMode($zoom, string $layout='default')
{
...
public function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if ($uni) {
        ...
        $sbarr = empty($this->AliasNbPages) ? range(0,32) : range(0,57);
...
public function Image(string $file, $x=null, $y=null, $w=0, $h=0, $type='', $link='')
{
...
public function _checkoutput()
{
	...
	{
		$this->Error(sprintf("Some data has already been output, can't send PDF file (output started at %s:%d)", $file, $line));
	}
 ...
 * @return mixed[]
 */
public function _loadfont(string $font): array
{
...
public function _textstring($s): string
{
	// Format a text string
	return '('.$this->_escape($s).')';
}

public function _UTF8toUTF16($s): string
{
...
public function _dounderline($x, $y, $txt): string
{
	// Underline text
...
public function _parsejpg(string $file): array
{
 ...
 * @return mixed[]
 */
public function _parsepng(string $file): array
{
...
public function _parsepngstream($f, string $file): array
{
	// Check signature
...
public function _readstream($f, $n): string
{
...
public function _parsegif(string $file)
{
...
public function _putstream(string $s)
{
	$this->_out('stream');
	$this->_out($s);
	$this->_out('endstream');
...
public function _putTTfontwidths(&$font, $maxUni) {
	...
	for ($cid=$startcid; $cid<$cwlen; $cid++) {
		if ($cid == 128 && !file_exists($font['unifilename'].'.cw127.php') && is_writable(dirname($this->_getfontpath().'unifont/x'))) {
 ...
 */
public function UTF8StringToArray($str): array {
   $out = [];
   $len = strlen($str);
   for ($i = 0; $i < $len; $i++) {
	$uni = -1;
