...
for ($i=0;$i<$nb_pa;$i++){
 ...
 $pdf->SetY(4);
 $pdf->SetX(120);
 $pdf->MultiCell(50,6,sprintf('%s: %s', $lang_date, $date_dev),1,'C',1);//
 ...
 $pdf->SetY(11);
 $pdf->SetX(120);
 $pdf->Cell(65,6,sprintf('%s : %s', $lang_de_num, $num_dev),1,0,'C',1);
...
if ($_POST['mail']=='y') {
 ...
 if ($newmail->sendfile()) {
     ...
     echo sprintf("<html><h3 style='color:red;'>%s</h3><script>setTimeout(function(){window.location='../lister_devis.php'},2000);</script></html>", $lang_env_par_mail_non);
 }
} else {
