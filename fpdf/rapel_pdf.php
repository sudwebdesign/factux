...
$pdf->SetY(268);
$pdf->SetX(30);
$pdf->MultiCell(160,4,$lang_condi_ven . PHP_EOL,0,'C',0);
//on enregisre les rappels
if(is_array($choix)){//Si facture(s)
 $sql1 = "UPDATE " . $tblpref .sprintf("facture SET r%s ='", $rapel_num).$date_jour."' WHERE num = '".$choix[0]."'";
 $counter = count($choix);
 for ($i=1 ; $i < $counter ; $i++){
  if (isset($choix[$i]) ){
   $sql1 .= " OR num = '".$choix[$i]."'";
...
echo sprintf("<html><script>window.location='%s';</script></html>", $file);
