...
$sql1 = "INSERT INTO " . $tblpref .sprintf("lot(prod, actif, date) VALUES ('%s', 'oui', '%s-%s-%s')", $prod, $annee, $mois, $jour);
mysql_query($sql1) || die('Erreur SQL1 !<br>'.$sql1.'<br>'.mysql_error());
$num_lot = mysql_insert_id();//le numero de lot

for ($i=1; $i<13; $i++) {#2015
 if($ing[$i]!=''){
 $sql3 = "INSERT INTO " . $tblpref .sprintf("cont_lot(num_lot, ingr, fourn, fourn_lot) VALUES ('%s', '", $num_lot).$ing[$i]."', '".$four[$i]."', '".$lot_four[$i]."')";
 mysql_query($sql3) || die('Erreur SQL3 !<br>'.$sql3.'<br>'.mysql_error());
 }
}
