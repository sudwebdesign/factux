...
function nombre_literal($nombre, $partie_entiere=ENTIER, $virgule=VIRGULE, $devise=DEVISE, $decimale=DECIMALE){
 ...
 $millier = array("", "mille","million","milliard","billion","trillion","quadrillion","quintillion"); // on fait de cette valeur un nombre
 ...
 // en dessous de cent
  if ($les_dizaines*10 + $lunite == 0){
   ...
   $r .= $dizaine[$les_dizaines].S;#pluriel
 ...
 // cela couvre largement les nombres classiques. 
  ...
  // pour les milliers
  $milliers = $millier;#array("", "mille","million","milliard","billion","trillion","quadrillion","quintillion");
  foreach ($bloc as $b) {
   $s = S;#pluriel
   $rc = nombre_literal($b);
