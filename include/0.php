...
if (version_compare(phpversion(),'7', '>=')) {
    include_once(__DIR__ . '/7mysql.php');
}#non testé (Fixed) TEP
...
if(version_compare(phpversion(),'5.4.0','>=')){#function fix_session_register(){
 function session_register(...$args): void{foreach ($args as $key){
   $_SESSION[$key]=@$GLOBALS[$key];#var_dump($_SESSION[$key],$GLOBALS,$args);
  }
 }

 function session_is_registered($key): bool{
  return isset($_SESSION[$key]);
 }

 function session_unregister($key): void{
  unset($_SESSION[$key]);
 }
}
