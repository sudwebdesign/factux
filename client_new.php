...
if($pass != $pass2){
 ...
 include(__DIR__ . '/form_client.php');
 exit;
}
...
if($nom=='' || $rue=='' || $ville=='' || $code_post=='' || $num_tva==''){
 ...
 include(__DIR__ . '/form_client.php'); // On inclus le formulaire d'identification
 exit;
}
...
$sql1 = "INSERT INTO " . $tblpref .sprintf("client(nom, nom2, rue, ville, cp, num_tva, login, pass, mail, civ, tel, fax) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')", $nom, $nom_sup, $rue, $ville, $code_post, $num_tva, $login, $pass, $mail_cli, $civ, $tel, $fax);
mysql_query($sql1) || die('Erreur SQL !<br>'.$sql1.'<br>'.mysql_error());
$message='';
if ($login != '' && $pass2 != '' && $mail_cli != ''){
 $to = $mail_cli;
 $from = $mail_admin ;
 $subject = $lang_cre_mo_pa ;
 $message =  sprintf("%s %s <br>%s %s <br>%s <a href='mailto:", $lang_mai_cre, $login, $lang_mai_cr_pa, $pass2, $lang_mai_cre_enc).$mail_admin.sprintf("'>%s</a> %s", $lang_admini, $lang_pass_nou);
 if (courriel($to,$subject,$message,$from,$logo)) {
...
$message = sprintf('<h2>%s</h2>', $lang_client_enr).$message;
